...
(function() {
  ...
  Transform = {
    ...
    viewerToAtlas: function(coords) {
      ...
      matrix = [[108, 0, 0, -108], 
                [0, -147, 0, 108], 
                [0, 0, -108, 147]];
      //matrix = [[216, 0, 0, -108], [0, -296, 0, 108], [0, 0, -216, 147]];
      return this.transformCoordinates(coords, matrix);
    },
    atlasToViewer: function(coords) {
      var matrix;
      matrix = [[1.0 / 108, 0, 0, 0.0], 
                [0, -1.0 / 147, 0, 54.0 / 147], 
                [0, 0, -1.0 / 108, 74.0 / 108]];
      //matrix = [[1.0 / 216, 0, 0, 0.5], [0, -1.0 / 296, 0, 109.0 / 296], [0, 0, -1.0 / 216, 147.0 / 216]];
      return this.transformCoordinates(coords, matrix, false);
    },
    atlasToImage: function(coords) {
      var matrix;
      matrix = [[-1.0, 0, 0, 0],  //108 //pos
                [0, 1.0, 0, 39],  //40 
                [0, 0, 1.0, -39]]; //40 
      //matrix = [[-0.5, 0, 0, 54], [0, 0.5, 0, 74], [0, 0, 0.5, 54]];
      return this.transformCoordinates(coords, matrix);
    },
    imageToAtlas: function(coords) {
      var matrix;
      matrix = [[-1, 0, 0, 54], //-54 //pos
                [0, 1, 0, -91], //-91
                [0, 0, 1, -48]]; //-48
      //matrix = [[-2, 0, 0, -108], [0, 2, 0, -148], [0, 0, 2, -108]];
      return this.transformCoordinates(coords, matrix);
    }
  };
  ...
  ColorMap = (function() {
    ...
    $.extend(ColorMap.PALETTES, {
      ...
      'intense red': ['#C77986', '#D46E7F', '#E25F75', '#ED556E', '#F54C69', '#FE4463', '#FF0000'],
      'intense blue': ['#85a3b2', '#0000FF'],
      'red-blue': ['#C77986','#C77986','#0000FF','#0000FF', '#FF0000', '#FF0000', '#85a3b2','#85a3b2'],
